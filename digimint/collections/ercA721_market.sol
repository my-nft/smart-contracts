...
contract NonFungibleToken is Ownable, ERC721A, ERC721APausable {
    ...
    uint private _maxToMint = 1000;//---------------
    uint private _mintFee = 5e15;//---------------
    uint private _maxPerTransaction = 20;//---------------
     ...
     * See {ERC721-tokenURI}.
     */
    constructor(string memory name, string memory symbol, string memory baseURI_) public payable ERC721A(name, symbol) {
  ...
  /*
    ...
    */
    ...
    function mintOperation(address to, uint256 count, string memory ipfsId) internal {
        ...
        ipfs[_tokenIdTracker.current()] = ipfsId;
        
    }

    function mint(uint256 count, string memory ipfsId) payable public { //---------------
        mintOperation(_msgSender(), count, ipfsId); //---------------
    }
 
    function mint(address to, uint256 count, string memory ipfsId) payable public { //---------------
        mintOperation(to, count, ipfsId); //---------------
    }

    function mintGasFreeWhitelist(uint256 count,bytes32 leaf, bytes32[] memory _proof, uint256[] memory positions, string memory ipfsId) payable public {//---------------
