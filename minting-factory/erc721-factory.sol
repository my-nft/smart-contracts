...
contract NonFungibleToken is Ownable, ERC721A, ERC721APausable {
     ...
     * See {ERC721-tokenURI}.
     */
    constructor(string memory name, string memory symbol, string memory baseURI_, uint mintFee, uint maxToMint, uint256 maxPerWallet, uint maxPerTransaction, address _owner) public  ERC721A(name, symbol) {

        // require(msg.value >= deploymentFee, "ERROR: must pay required fees");

        transferOwnership(_owner);
        fundsWallet = payable(_owner);
