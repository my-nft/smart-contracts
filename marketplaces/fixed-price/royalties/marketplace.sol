...
contract NFT_Market is Ownable {
    ...
    address public constant trustedFeeTokenAddress = 0xCcEB2304a79dB04c95AE22716D7DE5D1a37D810b;
    address public constant trustedNftAddress = 0xB399eCc328b629c0c424478a5e89f08e07015171;
    ...
    uint public maxRoyalties = 500; // max per 1000, 1 means 0.1 %
    ...
    function setMaxRoyalties(uint _maxRoyalties) public onlyOwner {
    ...
    function mintNative(uint256 _royalties, address _creator) payable public {
        // owner can mint without fee
        // other users need to pay a fixed fee in token
        require(msg.sender == owner || msg.sender == admin, "Not allowed !");
        require(_royalties < maxRoyalties, "max royalties exceeded !");
        ...
        uint256 nftId = IERC721(trustedNftAddress).totalSupply();
        royalties[nftId] = _royalties;
        creators[nftId] = _creator;

        IERC721(trustedNftAddress).mint(_creator);
    ...
    function buy(uint tokenId) public payable {
        ...
        address payable _owner = address(uint160(owner));
        address payable _creator = address(uint160(creators[tokenId]));

        uint price = nftsForSalePrices[tokenId];
        uint fee = price.mul(sellingFeeRateX100).div(1e4);
        uint royaltiesAmount = price.mul(royalties[tokenId]).div(1e3);
        uint amountAfterFee = price.sub(fee);
        amountAfterFee = amountAfterFee.sub(royaltiesAmount);
        ...
        if (_priceType == PriceType.ETHER) {
            ...
            _owner.transfer(fee);
            _creator.transfer(royaltiesAmount);
