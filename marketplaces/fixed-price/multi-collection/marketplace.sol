...
contract NFT_Market is Ownable {
    ...
    address public constant trustedFeeTokenAddress = 0x267022751E06d97B9eE4e5f26cc1023670BDB349;
    address public constant trustedNftAddress = 0x6B16363D5930c6f27aF29397b77fFEC19170AA20;

    mapping(uint => address) public nftsCollections; // collection id to collection address
    mapping(address => uint) public nftsCollectionsIds; // collection address to collection id
    ...
    mapping (address => EnumerableSet.UintSet) private nftsForSaleIdsPercollection;

    // nft id => nft price
    mapping (uint => uint) private nftsForSalePrices;
    mapping(address => mapping (uint => uint)) private nftsForSalePricesPerCollection;
    // nft id => nft owner
    mapping (uint => address) private nftOwners;
    mapping (address => mapping (uint => address)) private nftOwnersPercollection;
    // nft id => ETHER | TOKEN
    mapping (uint => PriceType) private priceTypes;
    mapping (address => mapping (uint => PriceType)) private priceTypesPerCollection;

    // nft owner => nft id set
    mapping (address => EnumerableSet.UintSet) private nftsForSaleByAddress;

    mapping (address => mapping (address => EnumerableSet.UintSet)) private nftsForSaleByAddressPerCollection;
    ...
    function balanceOfPerCollection(address _collection, address owner) public view returns (uint256) {
        require(owner != address(0), "ERC721: balance query for the zero address");
        return nftsForSaleByAddressPerCollection[_collection][owner].length();
    }
    ...
    function totalListedPerCollection(address _collection) public view returns (uint256) {
        return nftsForSaleIdsPercollection[_collection].length();
    }
    ...
    function getTokenFromCollection(address _collection, uint tokenId) public view returns (uint _tokenId, uint _price, address _owner, PriceType _priceType) {
        _tokenId = tokenId;
        _price = nftsForSalePricesPerCollection[_collection][tokenId];
        _owner = nftOwnersPercollection[_collection][tokenId];
        _priceType = priceTypesPerCollection[_collection][tokenId];
    }
    ...
    function getTokensPerCollection(address _collection, uint startIndex, uint endIndex) public view returns
        (uint[] memory _tokens, uint[] memory _prices, address[] memory _owners, PriceType[] memory _priceTypes) {
        require(startIndex < endIndex, "Invalid indexes supplied!");
        uint len = endIndex.sub(startIndex);
        require(len <= totalListedPerCollection(_collection), "Invalid length!");
        ...
        for (uint i = startIndex; i < endIndex; i = i.add(1)) {
            ...
            uint tokenId = nftsForSaleIdsPercollection[_collection].at(i);
            uint price = nftsForSalePricesPerCollection[_collection][tokenId];
            address nftOwner = nftOwnersPercollection[_collection][tokenId];
            PriceType priceType = priceTypesPerCollection[_collection][tokenId];
    ...
    function getTokensPerCollection(address _collection, address seller, uint startIndex, uint endIndex) public view returns
        (uint[] memory _tokens, uint[] memory _prices, address[] memory _owners, PriceType[] memory _priceTypes) {
        // require(startIndex < endIndex, "Invalid indexes supplied!");
        uint len = endIndex - startIndex;
        // require(len <= balanceOfPerCollection(_collection, seller), "Invalid length!");
        ...
        for (uint i = startIndex; i < endIndex; i = i+1) {
            uint listIndex = i - startIndex;

            uint tokenId = nftsForSaleByAddressPerCollection[_collection][seller].at(i);
            uint price = nftsForSalePricesPerCollection[_collection][tokenId];
            // address nftOwner = nftOwnersPercollection[_collection][tokenId]; // commented because stack too deep
            PriceType priceType = priceTypesPerCollection[_collection][tokenId];

            _tokens[listIndex] = tokenId;
            _prices[listIndex] = price;
            _owners[listIndex] = nftOwnersPercollection[_collection][tokenId];
            _priceTypes[listIndex] = priceType;
        }
    }
    ...
    function registerCollection(address _collection) public {
        // owner can mint without fee
        // other users need to pay a fixed fee in token
        nftsCollections[collectionsNumber] = _collection;
        nftsCollectionsIds[_collection] = collectionsNumber;

        collectionsNumber = collectionsNumber.add(1);
    }
    ...
    function mintNativeItemForCollection(address _collections) payable public {
        // owner can mint without fee
        // other users need to pay a fixed fee in token

        require(IERC721(_collections).owner() == msg.sender, "Only collection owner can mint");
        address payable _owner = address(uint160(owner));
        if (msg.sender != owner) {
            require(msg.value >= mintFeeNative, "Insufficient fees");
            _owner.transfer(mintFeeNative);
        ...
        IERC721(_collections).mint(msg.sender);
    }

    function listFromCollection(address _collection, uint tokenId, uint price, PriceType priceType) public {
        IERC721(_collection).transferFrom(msg.sender, address(this), tokenId);

        nftsForSaleIdsPercollection[_collection].add(tokenId);
        nftsForSaleByAddressPerCollection[_collection][msg.sender].add(tokenId);
        nftOwnersPercollection[_collection][tokenId] = msg.sender;
        nftsForSalePricesPerCollection[_collection][tokenId] = price;
    ...
    function unlistForCollection(address _collection, uint tokenId) public {
        require(nftsForSaleIdsPercollection[_collection].contains(tokenId), "Trying to unlist an NFT which is not listed yet!");
        address nftOwner = nftOwnersPercollection[_collection][tokenId];
        require(nftOwner == msg.sender, "Cannot unlist other's NFT!");

        nftsForSaleIdsPercollection[_collection].remove(tokenId);
        nftsForSaleByAddressPerCollection[_collection][msg.sender].remove(tokenId);
        delete nftOwnersPercollection[_collection][tokenId];
        delete nftsForSalePricesPerCollection[_collection][tokenId];
        delete priceTypesPerCollection[_collection][tokenId];
    ...
    function buyFromCollection(address _collection,uint tokenId) public payable {
        require(nftsForSaleIdsPercollection[_collection].contains(tokenId), "Trying to unlist an NFT which is not listed yet!");
        address payable nftOwner = address(uint160(nftOwnersPercollection[_collection][tokenId]));
        address payable _owner = address(uint160(owner));

        uint price = nftsForSalePricesPerCollection[_collection][tokenId];
        uint fee = price.mul(sellingFeeRateX100).div(1e4);
        uint amountAfterFee = price.sub(fee);
        PriceType _priceType = priceTypesPerCollection[_collection][tokenId];

        nftsForSaleIdsPercollection[_collection].remove(tokenId);
        nftsForSaleByAddressPerCollection[_collection][nftOwnersPercollection[_collection][tokenId]].remove(tokenId);
        delete nftOwnersPercollection[_collection][tokenId];
        delete nftsForSalePricesPerCollection[_collection][tokenId];
        delete priceTypesPerCollection[_collection][tokenId];
